...
## 設計・コード品質ルール
設計とコードは以下の4つの観点を重視する：

### 予測しやすい
- 名前を見て何をするクラス・メソッドなのかが判断できる
- ファイル名、クラス名、メソッド名は機能を明確に表現する
- 一貫した命名規則を使用する

### 再利用しやすい
- さまざまな場所で利用できる設計にする
- 汎用的なロジックはモジュールやヘルパーに切り出す
- 依存関係を最小限にして結合度を下げる

### 保守しやすい
- 定義されたものがどこにあるかわかりやすい
- 関連するコードは同じ場所にまとめる
- 責任の分離を明確にする

### 拡張しやすい
- 再利用する際に部分的に異なるカスタマイズが簡単
- オープン・クローズド原則を適用する
- 継承やミックスインを適切に活用する

### ファイルサイズルール
- **一つのファイルは200行以下**を目標とする
- 200行を超える場合は責任ごとに分割を検討する
- 分割できる場合は迷わず分割する
