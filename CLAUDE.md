...
## プロジェクト概要
マチダのブログシステム - Rails 8.0.3 + Ruby 3.4.5 + Node.js v22.17.1

## CSS/HTMLクラス命名規則
- `.a--` - Atomic Design の Atom レベルの小さなパーツ用クラス（ボタン、入力欄など）
- `.l--` - レイアウト用クラス（配置、余白、グリッドなど）
- `.spec--` - テスト用クラス（テストでの要素選択用）
- **重要**: `.spec--`クラスはCSSのセレクタには使用しない（テスト専用）
...
- **ファイルの最後に空行を入れる**
- **コミットは小まめに行う（作業単位を細かく分ける）**
- **コミット前に必ず専用ブランチを切る（mainで直接コミットしない）**
- **コミット前に全テストを通し、失敗時は原因を解消してから進む**
- **フレーキーなテストを見つけたらその場で安定化させる（スキップで逃げない）**
- **タスクは常にメモに残し、いつでも作業を再開できる状態に保つ**
